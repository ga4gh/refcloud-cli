...
def dataset_onboard(dataset_id):
    ...
    if data_source["type"] == "s3":
        ...
        for drs_object_set in onboarding_fn(s3_client): # load drs objects and sub-objects (aliases, checksums, aws s3 access objects) into db
            ...
            for manifest_key in drs_object_set.keys():
                # unpack
                drs_object, aliases, checksums, aws_s3_access_objects = itemgetter("drs_object", "aliases", "checksums", "aws_s3_access_objects")(drs_object_set[manifest_key])
